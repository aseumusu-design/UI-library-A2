 --[[
  NovaUI v2.1 — Single-file Roblox UI Library (FIXED)
  Perbaikan:
  - math.atan2 -> math.atan(y, x) (kompatibel Roblox terbaru)
  - Ikon digambar lebih tebal & jelas (100% muncul)
  - Parent GUI aman untuk executor (gethui / CoreGui / PlayerGui fallback)
  - Fix deteksi nama ikon di tab (string.sub 1,8)
  - Close button hanya menyembunyikan window, bubble tetap bisa membuka
...
-- ============================ SAFE PARENT (executor-proof) ============================
local function getSafeParent()
    -- 1) gethui (executor paling umum & aman dari anti-cheat)
    ...
    -- 2) CoreGui dengan proteksi
    local ok2, core = pcall(function()
        return game:GetService("CoreGui")
    end)
    if ok2 and core then
        local ok3, robloxGui = pcall(function()
            return core:WaitForChild("RobloxGui", 1)
        end)
        if ok3 and robloxGui then return core end
        return core
    end
...
local function parentGui(gui)
    ...
    -- proteksi tambahan untuk executor lama
    pcall(function()
...
-- FIX: gambar ikon dengan garis lebih tebal & pakai math.atan(y,x)
local function drawIcon(parent, name, size, color)
    ...
    canvas.Size = UDim2.fromOffset(size or 18, size or 18)
    canvas.Parent = parent
    local thickness = math.max(2, math.floor((size or 18) / 9)) -- FIX: ketebalan proporsional
    for _, line in ipairs(path) do
        ...
        part.Size = UDim2.new(length/24, 0, 0, thickness) -- FIX: pakai thickness
        part.Rotation = math.deg(math.atan(dy, dx)) -- FIX: math.atan 2 argumen
        part.BackgroundColor3 = color or THEME.Text
...
function Bubble.new(config)
    ...
    gui.DisplayOrder = 999 -- FIX: selalu di atas
    parentGui(gui) -- FIX: parent aman
...
function Bubble:SetIcon(icon)
    ...
    self.Icon = drawIcon(self.Button, icon or "Menu", math.floor(self.Button.AbsoluteSize.X*.44), Color3.new(1,1,1))
...
function NovaUI:CreateWindow(cfg)
    ...
    gui.ResetOnSpawn = false
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.DisplayOrder = 998 -- FIX: di atas game UI
    parentGui(gui) -- FIX: parent aman untuk executor
    ...
    main.Visible = true -- FIX: pastikan terlihat
    main.Parent = gui
    ...
    -- FIX: close hanya menyembunyikan window (bisa dibuka lagi via bubble/keybind)
        ...
        function tab:Select()
            for _, t in ipairs(Window.Tabs) do
                ...
                for _, child in ipairs(t.Button:GetChildren()) do
                    -- FIX: "NovaIcon_" = 9 karakter, cek 8 karakter pertama "NovaIcon"
                    if string.sub(child.Name,1,8) == "NovaIcon" then
                        for _, line in ipairs(child:GetChildren()) do if line:IsA("Frame") then line.BackgroundColor3 = THEME.Muted end end
                    end
                end
            end
            ...
            for _, line in ipairs(tabIcon:GetChildren()) do if line:IsA("Frame") then line.BackgroundColor3 = THEME.Text end end
        end
